...
4. The Fetch services button shall call `getServices` and store the returned list (filtered to `type` 0 = local tags or 5 = all known tags) into `hydrusAvailableTagServices` as `{ name, service_key }` pairs.
