...
9. Each tile shall expose a `⋮` button that opens a context menu with at least: a tag list (filtered through `hydrusIgnoredTagPatterns`, sorted alphabetically via `localeCompare`, and rendered as wrap-on-row chips inside a menu whose max width is fixed in CSS so individual tags never break mid-string), a Copy tags action whose payload is the same alphabetically-sorted, comma-joined list, a Copy image reference action (titled `Copy video reference` when the tile MIME is `video/*`) that writes the Markdown string `[<label>](hydrus://<hash>)` to the clipboard — where `<label>` is `layerLabelFromTags(tile.knownTags, tile.hash)` — and shows the Notice `Reference copied.`, and a `Set as background` action that works for both images and videos. The Copy image/video reference action is always present; Copy tags is shown only when the tile has at least one non-ignored tag.
