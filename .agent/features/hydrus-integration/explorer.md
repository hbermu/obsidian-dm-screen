...
9. Each tile shall expose a `⋮` button that opens a context menu with at least: a tag list (filtered through `hydrusIgnoredTagPatterns`, sorted alphabetically via `localeCompare`, and rendered as wrap-on-row chips inside a menu whose max width is fixed in CSS so individual tags never break mid-string), a Copy tags action whose payload is the same alphabetically-sorted, comma-joined list, and a `Set as background` action that works for both images and videos.
