...
- `src/map/foundry.ts` — `parseNedb`, `parseLeveldbLog`, `foundrySceneToWalls`, `pickScene`, `parseFoundryModule`, `FoundryScene`, `FoundryImportResult`; bundled runtime dependency: `fflate` (used for `unzipSync` in `MapFogModal`)
...
37. `parseFoundryModule(entries: Record<string, Uint8Array>): FoundryImportResult` (in `src/map/foundry.ts`) shall: gather documents from every `.db` entry via `parseNedb` (TextDecoder); gather documents from every `.log`/`.ldb` entry under a `packs` path via `parseLeveldbLog`; call `pickScene`; throw `new Error("No scene with walls found in module")` when no scene is found; return `{ walls: scene.walls (in scene px), gridSquares: { x: width/gridSize, y: height/gridSize }, pixelsPerGrid: gridSize }`.
...
39. `parseLeveldbLog(bytes: Uint8Array): unknown[]` shall parse the LevelDB write-ahead-log: 32768-byte blocks; each record header is `crc(4 LE) + length(2 LE) + type(1)` (CRC is not verified); types 1=FULL (emit immediately), 2=FIRST / 3=MIDDLE / 4=LAST (concatenate, then emit); stop a block when a zero-length/zero-type header appears; for each reassembled record, locate the first `{` and parse the balanced top-level JSON object via brace-depth walking (respecting strings and escapes).
40. `foundrySceneToWalls(scene): FoundryScene | null` shall: require numeric `width`, `height`, array `walls`, and `grid` that is either a number or `{ size: number }`; map each wall `w.c = [x1,y1,x2,y2]`; filter walls where `sense === 0`; skip zero-length walls (`x1===x2 && y1===y2`); for `door > 0` produce `{ x1,y1,x2,y2,door:true, ...(ds===1 ? {open:true} : {}) }`; return null when no valid walls or invalid dims.
