...
40. `foundrySceneToWalls(scene): FoundryScene | null` shall: require numeric `width`, `height`, array `walls`, and `grid` that is either a number or `{ size: number }`; map each wall `w.c = [x1,y1,x2,y2]`; filter walls that do not block sight (`sense === 0` in older Foundry, `sight === 0` in newer); skip zero-length walls (`x1===x2 && y1===y2`); for `door > 0` produce `{ x1,y1,x2,y2,door:true, ...(ds===1 ? {open:true} : {}) }`; return null when no valid walls or invalid dims.
