...
11. `republishToServer()` shall re-broadcast `map-calibration` plus, when a map is active, `map-show`/`map-config`/`map-view`/`map-aoe-sync`, so map screens connecting after a server restart reconstruct the scene; the DM panel shall restore its map state from the persisted late-joiner cache on open.
...
13. In physical mode, `clampPan` shall prevent the viewport from extending past the map edges: the pan is constrained so that no black (empty) region is visible. When the map is smaller than the viewport on an axis, the pan is forced to the centre of that axis.
14. The DM panel shall expose an AoE Overlays section (visible when a map is active) with: an Add AoE menu offering named presets (Fireball 20 ft circle, Spirit Guardians 15 ft circle) and a Custom entry; per-AoE controls (color, opacity, rotation, delete); and a Clear All button.
15. Each AoE shall be rendered on the map screen canvas as a filled+stroked shape (circle, square, cone from vertex at 53°, or line/rectangle) at the correct map-pixel position, transformed by the same scale/translation as the grid.
16. In the DM pan preview, each AoE shall appear as a draggable colored dot; dragging repositions the AoE and broadcasts `map-aoe-sync` in real time.
