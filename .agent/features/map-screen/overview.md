...
11. `republishToServer()` shall re-broadcast `map-calibration` plus, when a map is active, `map-show`/`map-config`/`map-view`/`map-fog` (and `map-aoe-sync` per `aoe-overlays.md` requirement 10), so map screens connecting after a server restart reconstruct the scene; the DM panel shall restore its map state — including AoEs and fog data URL — from the persisted late-joiner cache on open.
