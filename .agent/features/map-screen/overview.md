...
16. In the DM pan preview (both scale modes), each AoE shall appear as a draggable colored dot; dragging repositions the AoE and broadcasts `map-aoe-sync` throttled to the same cadence as pan drags, with an immediate broadcast on release. Opacity slider drags are throttled the same way.
