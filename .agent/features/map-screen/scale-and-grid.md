...
6c. The preview stage shall render rotated to the view rotation (`rotate(θ)`) and scaled by a DM-local preview zoom, so the DM sees the TV's actual orientation; every screen→map conversion (click-to-center, rect drag, AoE dot drag, rotation handle) divides by `baseScale × previewZoom` and un-rotates through the inverse rotation. Preview zoom (wheel toward the cursor or the overlay zoom slider, 1×–6×, 0.1 steps), middle-drag panning, and the ⌂ center-and-reset button are local to the DM panel — never broadcast, reset when a new map is applied.
6d. A lock toggle (🔓/🔒) in the preview overlay shall, while locked, block every map-pan interaction from the preview (click-to-center and viewport-rect drag; the rect turns dashed and inert) so the DM can work on AoEs without disturbing what the players see. AoE dot/handle drags, preview zoom, and middle-drag panning stay available; the lock is DM-local, never broadcast, and persists across re-renders for the panel's lifetime.
