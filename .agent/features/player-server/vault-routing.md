...
5. If `getAbstractFileByPath` returns nothing, then `readVaultBytes` shall fall back to `vault.adapter.exists` + `vault.adapter.readBinary`, so dotfolders (e.g. `.dm-screen/hydrus/...`) are reachable.
