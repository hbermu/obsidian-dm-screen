...
3. The release workflow reads the labels, computes the next version from the latest stable tag (not `manifest.json` — see below), bumps the three version files in a detached commit, tags `vX.Y.Z`, builds `main.js`, publishes a GitHub release with tier-shaped notes (see below), and appends a matching section to `CHANGELOG.md` on `main`.
...
| `minor` | Categorized aggregation (Features / Fixes / Documentation / Build & CI / Other) of every PR merged since the **previous minor** (`v*.*.0`), not just the previous stable. Patches between minors are folded in. |
| `major` | The PR body of the merge that bumps to `vX.0.0` (the workflow strips a trailing `## Test plan` section if present), followed by a bulleted list of every minor (`v*.*.0`) shipped since the previous major, a link to `CHANGELOG.md`, and a `Full Changelog` compare URL. |
...
### CHANGELOG.md is workflow-owned

The root `CHANGELOG.md` is regenerated by the workflow after every stable release: the same release-notes body that ships to GitHub is prepended as a new `## vX.Y.Z — <tier> — <YYYY-MM-DD>` section under the `# Changelog` header. The workflow then commits as `chore(changelog): update for vX.Y.Z` and pushes directly to `main`. This direct push:

- Does NOT re-fire the release workflow (the triggers are `pull_request: closed` and `push: branches-ignore: [main]`).
- Requires the repo branch-protection rules to allow `github-actions[bot]` to bypass required reviews/checks on `main`. If the push fails with "protected branch" the rule needs adjusting — the file is then out of sync with reality until the next successful publish.

**Do not edit `CHANGELOG.md` by hand.** Any manual change is overwritten on the next release. To regenerate the file from history (e.g. after re-importing a release), run `scripts/bootstrap-changelog.sh` — it rebuilds the file from `gh release list` end to end.
