...
The root `CHANGELOG.md` is regenerated by the workflow after every stable release: the same release-notes body that ships to GitHub is prepended as a new `## X.Y.Z — <tier> — <YYYY-MM-DD>` section under the `# Changelog` header. The workflow then commits as `chore(changelog): update for X.Y.Z` and pushes directly to `main`. This direct push:
