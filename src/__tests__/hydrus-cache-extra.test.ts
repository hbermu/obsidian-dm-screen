...
describe("HydrusCache - paths", () => {
  it("generates correct vault paths", () => {
    ...
    const cache = new HydrusCache(null, { folder: ".dm-screen/hydrus", ttlDays: 30, adapter });
    const paths = cache.paths("abcdef123", "webp");
    expect(paths.vaultPath).toBe(".dm-screen/hydrus/abcdef123.webp");
    expect(paths.thumbVaultPath).toBe(".dm-screen/hydrus/abcdef123.thumb.jpg");
  });
  ...
  it("uses default folder when empty", () => {
    ...
    const paths = cache.paths("abc", "png");
    expect(paths.vaultPath).toBe(".dm-screen/hydrus/abc.png");
  });
});
...
describe("HydrusCache - concurrent operations", () => {
  ...
  beforeEach(() => {
    ...
    cache = new HydrusCache(null, { folder: ".dm-screen/hydrus", ttlDays: 30, adapter });
  });
...
describe("HydrusCache - index resilience", () => {
  it("recovers from corrupted index JSON", async () => {
    const adapter = new MemoryAdapter();
    adapter.text.set(".dm-screen/hydrus/index.json", "not valid json {{{");
  ...
  it("recovers from wrong version index", async () => {
    const adapter = new MemoryAdapter();
    adapter.text.set(".dm-screen/hydrus/index.json", JSON.stringify({ version: 99, entries: { x: {} } }));
  ...
  it("creates folder on first write if it doesn't exist", async () => {
    ...
    expect(mkdirSpy).toHaveBeenCalledWith(".dm-screen/hydrus");
  });
});
...
describe("HydrusCache - TTL edge cases", () => {
  it("ttlDays minimum is 1 day", () => {
    const adapter = new MemoryAdapter();
    const cache = new HydrusCache(null, { folder: ".dm-screen/hydrus", ttlDays: 0, adapter });
  ...
  it("sweep does not remove entries that are exactly at the TTL boundary", async () => {
    ...
    const idx = JSON.parse(adapter.text.get(".dm-screen/hydrus/index.json")!);
    const ttlMs = 30 * 24 * 60 * 60 * 1000;
    idx.entries["h1"].lastUsedAt = Date.now() - ttlMs + 1000; // just within TTL
    adapter.text.set(".dm-screen/hydrus/index.json", JSON.stringify(idx));
